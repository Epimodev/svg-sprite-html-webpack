...
module.exports = class SvgSpriteHtmlWebpackPlugin {
  ...
  constructor(options = {}) {
    ...
    this.pushSVGIncomingFile = this.pushSVGIncomingFile.bind(this);
    this.pushSvg = this.pushSvg.bind(this);
    this.processSvg = this.processSvg.bind(this);

    if (this.includeFiles.length) {
      this.includeFiles.map(e => {
        fs.readFile(e, (err, data) =>
          this.pushSVGIncomingFile(e, data, data.toString('utf-8')));
      });
    }
  }
    ...
    compiler.plugin('compilation', (compilation) => {
      compilation.plugin('normal-module-loader', (loaderContext) => {
        ...
        if (!loaderContext.svgList || !loaderContext.generateSymbolId) {
          ...
          loaderContext.pushSVGIncomingFile = this.pushSVGIncomingFile;
   ...
   * This function generate svg sprite with the list of svg collected by the svgLoader (./loader.js)
   * When the sprite is generated, it's injected in the HTML string
   * @param {string} filePath - represents the system path to the svg file
   * @param {Buffer} fileBuffer - a Node Buffer object from the file used inside the caching function
   * @param {string} fileContent - a string representation of the svg object
   */
  pushSVGIncomingFile(filePath, fileBuffer, fileContent) {
    const svgPath = filePath;
    const svgHash = XXHash.hash(fileBuffer, 0xCAFEBABE);
    const symbolId = this.generateSymbolId(svgPath, svgHash, fileContent);
